...
class HabitRepository(private val habitDao: HabitDao, private val executor: ExecutorService) {
    ...
    fun getHabits(filter: HabitSortType): LiveData<PagedList<Habit>> {
        val sortedQuery: SupportSQLiteQuery = SortUtils.getSorteredQuery(filter)
        val config = PagedList.Config.Builder()
            .setEnablePlaceholders(false)
            .setPageSize(10)
            .build()
        val habitDataSourceFactory = habitDao.getHabits(sortedQuery)
        return LivePagedListBuilder(habitDataSourceFactory, config).build()
    }
